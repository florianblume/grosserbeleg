...
\subsection{Incremental Learning} \label{subsection:experiments_online_learning}
...
%TODO looks like this gives better results -> 
	...
	\begin{subfigure}[t]{0.4\textwidth}
			\begin{tikzpicture}[scale=0.95]
                 ...
                 y label style={at={(axis description cs:-0.1,.5)},rotate=90,anchor=south},]
      			...
      			\addplot[smooth,tb_color_2] table [x=Step, y=Value, col sep=comma] {experiments/model1/exp5_1/train_loss.csv};
	...
	\begin{subfigure}[t]{0.4\textwidth}
			\begin{tikzpicture}[scale=0.95]
                 ...
                 y label style={at={(axis description cs:-0.1,.5)},rotate=90,anchor=south},]
      			...
      			\addplot[smooth,tb_color_2] table [x=Step, y=Value, col sep=comma] {experiments/model1/exp5_1/val_loss.csv};
    			\end{axis}
			\end{tikzpicture}
		\caption{Validation losses.}
	\end{subfigure}
	\caption{Training and validation losses of the active training method compared to training from scratch. The spike of the blue error curve in the left graph is where the training of the images having the smallest IoU values begins.}
...
It is difficult to compare the networks' performances, as to our knowledge there exist no RGB-only baseline benchmark of the T-Less dataset. Results of more methods is necessary to be able to properly compare the accuracies.
...
%TODO summarize what has been done

%TODO network can be used to predict initial positions -> rotation has to be adjusted manually